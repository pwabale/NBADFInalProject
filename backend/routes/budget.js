...
router.get("/",[auth],async(req,res)=>{
    let result = await Budget.find({userId: req.user._id});
    return res.send(result);
})
...
router.get("/:budgetTypeId",[auth],async(req,res)=>{
    ...
    let result = await Budget.find({userId: req.user._id, budgetTypeId: req.params.budgetTypeId});
    return res.send(result);
})
...
router.put("/:budgetId",[auth],async(req,res)=>{
    ...
    if(budgetToBeModified) {
        if(budgetToBeModified.userId == req.user._id) {
            let budget = await Budget.findOneAndUpdate({_id:req.params.budgetId}, {budgetAmount:req.body.budgetAmount}, {new:true});
            res.send(budget);
        } else {
